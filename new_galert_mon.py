...
def _add_total_change_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Add T_change% and T_change$ vs original cost (example_new_alerts.csv)."""
    out = df.copy()
    qty_col = next((c for c in ["qty", "Shares", "QTY", "Qty"] if c in out.columns), None)
    cost_col = next((c for c in ["per_share", "Original Cost", "Cost", "per share"] if c in out.columns), None)

    if out.empty or qty_col is None or cost_col is None or "Current Price" not in out.columns:
        out["T_change%"] = pd.Series(dtype=str)
        out["T_change$"] = pd.Series(dtype=str)
        return out

    qty = out[qty_col].map(_parse_number).astype("float64")
    cost = out[cost_col].map(_parse_number).astype("float64")
    current = pd.to_numeric(out["Current Price"], errors="coerce")
    gain = current - cost
    pct = (gain / cost * 100).where(cost.notna() & (cost != 0))
    dollars = gain * qty

    out["T_change%"] = pct.map(lambda v: f"{v:.2f}%" if pd.notnull(v) else "N/A")
    out["T_change$"] = dollars.map(lambda v: f"{v:.2f}" if pd.notnull(v) else "N/A")
    return out
...
class StockAlertMonitorGUI(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1280;720".replace(";", "x"))
    ...
    def _build_ui(self):
        ...
        self.columns = [
            ...
            ("t_change_pct", "T_change%", 100, "e"),
            ("t_change_dol", "T_change$", 100, "e"),
        ]
    ...
    def _run_scan_thread(self, input_path: str, daily_limit: float, monthly_limit: float):
        try:
            ...
            out_name = os.path.join(_app_dir(), f"{timestamp}_new_alerts.csv")

            if alerts.empty:
                alerts["short interest"] = pd.Series(dtype=str)
                alerts = _add_total_change_columns(alerts)
            ...
            extra_cols = ("short interest", "T_change%", "T_change$")
            price_cols = [c for c in alerts.columns if c not in holdings_cols and c not in extra_cols]
            ordered_cols = holdings_cols + ["short interest"] + price_cols + ["T_change%", "T_change$"]
    ...
    def _on_scan_complete(self, alerts_df: pd.DataFrame, out_file: str, total_syms: int, triggered_syms: int, as_of: str = ""):
        ...
        for display_idx, (_, row) in enumerate(alerts_df.iterrows()):
            ...
            t_pct_val = str(row.get("T_change%", "N/A"))
            t_dol_raw = row.get("T_change$", "N/A")
            t_dol_num = _parse_number(t_dol_raw)
            t_dol_val = f"${t_dol_num:,.2f}" if t_dol_num is not None else str(t_dol_raw)

            values = (
                sym_val, acct_val, qty_val, cost_val, si_val,
                cp_val, mp_val, d_val, m_val, t_pct_val, t_dol_val,
            )
